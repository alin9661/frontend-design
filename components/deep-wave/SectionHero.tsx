...
export default function SectionHero() {
  ...
  return (
    <section
      ...
      className="relative flex min-h-svh flex-col justify-center overflow-hidden px-6 text-forest"
    >
      ...
      {/* GL view host renders behind this content via EngineProvider's
          single shared <GlCanvas/> — nothing to mount per-section. Copy is
          left-aligned into its own column (confirmed design-review fix):
          lib/scenes/hero-can/scene.ts now anchors the can to the RIGHT
          (`CAN_X_OFFSET_FACTOR`) instead of dead-center, so type and product
          each get their own column instead of the can photobombing the
          sub-copy/footnote through the middle of the page. */}
      <div className="max-w-[560px]">
        ...
        <p className="mt-4 max-w-md font-body text-xs text-forest/50">
          *Marketing department&apos;s phrasing, not ours. Check the
          ingredient list — it contains no AI.
        </p>
        <GagStats
          stats={[
            {
